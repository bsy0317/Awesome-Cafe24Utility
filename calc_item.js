...
function checkCategory(){								//스크립트가 활성화돼야 하는지 확인하는 함수
	if((location.hostname === 'chingchanmall.com')||(location.hostname === 'ccnaramall.cafe24.com')){
	...
	}else if(location.hostname === 'm.chingchanmall.com'){
...
function redirectOtherSize(){				//현수막 상세페이지에서 기타사이즈를 선택하면 기타사이즈로 이동합니다.
	...
	if(checkItemCode() != true){
		console.log("기타사이즈를 선택하면 계산페이지로 Redirect 합니다.");
		$('select[option_title=현수막 사이즈선택]').change(function(e){
			...
			if(selectValue.includes("기타사이즈")){
				...
				window.location.href = "https://"+location.hostname+"/product/detail.html?product_no="+pId+"&name="+product_name+"&imgsrc="+originalImageSrc;
			}
		});
	}else{
		return;
...
function main(){
	if(checkCategory() != true) return;		//현수막 카테고리인가?
	if(checkItemCode() != true){
		console.log("지원가능한 현수막 계산 버전이 아닙니다.");
		return;
	}
...
setTimeout(function(){
	console.log("Hi SMART BANNER(CALC_ITEM.js) LOAD OK :)");
	main(); //실행
	redirectOtherSize();
},2000);
setTimeout(function(){
